...
| PROD-1 | (war) High | **Nur in Produktion (Docker/Node):** serverseitiges PDF schlug mit `ENOENT '/pdf/fonts/Arimo-Regular.woff'` fehl → E-Mail wurde nie versandt (Bestätigung „E-Mail nicht zugestellt"). Ursache: Server-Routen importierten `flexcoverPdfFilename` aus `@/lib/pdf` (index.ts), das beim Import die Arimo-Schrift mit **Browser-URL** `/pdf/fonts/…` registriert; im Node-Kontext als Dateipfad geöffnet. Zusätzlich verschluckte der äußere `catch` den Fehler stumm. | **Behoben (live):** Dateinamen-Helfer in seiteneffektfreies `@/lib/pdf/filename` ausgelagert; Routen importieren von dort; `catch`-Blöcke loggen jetzt die Ursache; Warnhinweis in `index.ts`. End-to-end in Prod verifiziert (E-Mail + PDF-Anhang). Von den Tests nicht abgedeckt, weil der Node-Test die index.ts-Browser-Registrierung nicht mit-importierte. |
